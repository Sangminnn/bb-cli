...
interface DiffViewerProps {
  ...
  onAddFileComment?: (file: string, body: string) => Promise<void>;
}
...
export const DiffViewer = memo(function DiffViewer({
  ...
  const handleAddComment = useCallback(
    async (
      ...
      try {
        await onAddComment(file.path, line, body, codeContent, side, reviewRequested);
      } catch (error) {
  ...
  const handleAddFileComment = useCallback(
    async (body: string) => {
      ...
      try {
        await onAddFileComment(file.path, body);
      } catch (error) {
        console.error('Failed to add file-level comment:', error);
      }
    },
    [file.path, onAddFileComment],
  );
  ...
  return (
    ...
    >
      ...
      {!isCollapsed && fileSummary && <FileSummaryCard summary={fileSummary} />}
