...
interface FileListProps {
  ...
  viewportFileIndex: number | null;
  onNavigateToComment?: (thread: CommentThread) => void;
}
...
export const FileList = memo(function FileList({
  ...
  const handleFileRowHover = (filePath: string, target: HTMLElement) => {
    if (!onNavigateToComment) return;
    if (!threadsByFile.get(filePath)?.length) return;

    cancelCloseTimer();
    cancelOpenTimer();

    const rect = target.getBoundingClientRect();
    const wouldOverflow = rect.right + POPOVER_GAP_PX + POPOVER_WIDTH_PX > window.innerWidth;
    const left = wouldOverflow
      ? Math.max(POPOVER_GAP_PX, rect.left - POPOVER_GAP_PX - POPOVER_WIDTH_PX)
      : rect.right + POPOVER_GAP_PX;

    openTimerRef.current = window.setTimeout(() => {
      setHoveredFile({ filePath, top: rect.top, left });
      openTimerRef.current = null;
    }, HOVER_OPEN_DELAY_MS);
  };

  const handleFileRowLeave = () => {
    cancelOpenTimer();
    scheduleClose();
  };
  ...
  const handlePopoverSelect = (thread: CommentThread) => {
    cancelOpenTimer();
    cancelCloseTimer();
    setHoveredFile(null);
    onNavigateToComment?.(thread);
  };
  ...
  const renderTreeNode = (node: TreeNode, depth: number = 0): React.ReactNode => {
    if (node.isDirectory && node.children) {
      ...
      const isInViewport = viewportFileIndex !== null && viewportFileIndex === fileIndex;
      ...
      return (
        <div
          ...
          className={`flex items-center gap-2 px-4 py-2 hover:bg-github-bg-tertiary cursor-pointer transition-colors ${
            ...
            isInViewport
              ? 'bg-github-accent/10 border-l-2 border-l-github-accent -ml-[2px]'
              : isSelected
          ...
          onMouseEnter={(event) => handleFileRowHover(file.path, event.currentTarget)}
          onMouseLeave={handleFileRowLeave}
        >
          <Checkbox
            checked={isReviewed}
            onChange={() => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {hoveredFile && onNavigateToComment && (
        <FileCommentPopover
          threads={threadsByFile.get(hoveredFile.filePath) ?? []}
          position={{ top: hoveredFile.top, left: hoveredFile.left }}
