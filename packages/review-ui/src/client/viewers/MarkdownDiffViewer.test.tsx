...
describe('MarkdownDiffViewer', () => {
  ...
  it('shows Full Preview tab only after prefetch succeeds', async () => {
    ...
    expect(screen.queryByRole('button', { name: '전체 미리보기' })).not.toBeInTheDocument();

    await waitFor(() => {
      expect(screen.getByRole('button', { name: '전체 미리보기' })).toBeInTheDocument();
    });
  ...
  it('renders Mermaid diagrams in Diff Preview', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Diff 미리보기' }));
