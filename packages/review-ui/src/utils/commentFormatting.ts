...
function formatCommentContent(body: string, codeContent?: string): string {
  if (hasSuggestionBlock(body)) {
    const suggestions = parseSuggestionBlocks(body).filter((b) => b.isSuggestion);
...
function formatCommentThreadPrompt(thread: CommentThread): string {
...
function formatAllCommentThreadsPrompt(threads: CommentThread[]): string {
