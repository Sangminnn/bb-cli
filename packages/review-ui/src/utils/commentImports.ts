...
function clonePosition(position: DiffCommentPosition): DiffCommentPosition {
  ...
  return {
    kind: position.kind,
    side: position.side,
    line: position.line === undefined ? undefined : cloneLineRange(position.line),
  };
}
...
function cloneMessage(message: DiffCommentMessage): DiffCommentMessage {
  return {
    ...
    appliedSuggestions: message.appliedSuggestions
      ? [...message.appliedSuggestions]
      : undefined,
    codeSnapshotAtCreation: message.codeSnapshotAtCreation,
  };
}
...
export function serializeCommentImports(commentImports: CommentImport[]): string {
  return JSON.stringify(
    commentImports.map((commentImport) => {
      const position = commentImport.position;
      const serializedPosition =
        position.kind === 'file'
          ...
          : {
              kind: position.kind,
              side: position.side,
              line: position.line === undefined ? undefined : serializeLineRange(position.line),
            };
