...
> = {
  en: {
    ...
    aiAgents: {
      ...
      desc: 'Submit review threads so a coding agent orchestrator can pick up the context',
    },
    responsive: {
      tab: 'mobile',
      label: 'responsive UI',
  ...
  ja: {
    ...
    aiAgents: {
      tab: 'AI',
      label: 'AIエージェント連携',
      desc: 'レビュースレッドをサブミットし、コーディングエージェントのオーケストレーターに引き渡す',
    },
    responsive: {
      tab: 'レスポン',
      label: 'レスポンシブ対応',
  ...
  ko: {
    ...
    aiAgents: {
      tab: 'AI',
      label: 'AI 에이전트 연동',
      desc: '리뷰 스레드를 제출하면 코딩 에이전트 오케스트레이터가 맥락을 가져갑니다',
    },
    responsive: {
      tab: '반응형',
      label: '반응형 대응',
